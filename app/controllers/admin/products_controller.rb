class Admin::ProductsController < Admin::BaseController
  ...
  def update_product_filter_value
    fv = @product.product_filters.find(params[:pfv_id])
    fv.update(catalog_filter_value_id: params[:new_id].to_i)
    current_user.record!(@product, 'Отредактировал', "Фильтр: <b>#{fv.filter.display_name}</b>: #{fv.filter_value.display_name}")
    head 200
  end
